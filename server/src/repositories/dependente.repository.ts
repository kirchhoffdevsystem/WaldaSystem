import { supabase } from "../db/supabase"
import type { Dependente } from "../types/types_db"

export default class DependenteRepository {
  async todos(): Promise<Dependente[] | null> {
    const { data, error } = await supabase
      .from('dependente')
      .select('*')

    if (error) {
      console.error('Erro ao buscar dependentes:', error)
      return null
    }
    return data
  }

  async criar(dependente: Omit<Dependente, 'id'>): Promise<Dependente | null> {
    const { data, error } = await supabase
      .from('dependente')
      .insert(dependente)
      .select('*') // retorna os campos do registro inserido
      .single(); // retorna um objeto único, não array

    if (error) {
      console.error('Erro ao cadastrar dependente:', error);
      return null;
    }
    return data;
  }

  async atualizarStatus(id: number, status: string) {
    const { data, error } = await supabase
      .from("dependente")
      .update({ status })
      .eq("id", id)
      .select()
      .single();

    if (error) {
      console.error("Erro ao atualizar status:", error);
      return null;
    }
    return data;
  }

  /* mostrar usuario a ao qual o dependente esta associado */

}